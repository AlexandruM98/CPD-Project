...
public class Config {
    ...
    public PubSubInboundChannelAdapter sportChannelAdapter(
        ...
        PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "Sport-sub3");
    ...
    @Bean
    public PubSubInboundChannelAdapter cookingChannelAdapter(
            @Qualifier("cookingInputChannel") MessageChannel inputChannel,
            PubSubTemplate pubSubTemplate) {
        PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "Cooking-sub3");
        adapter.setOutputChannel(inputChannel);
    ...
    @Bean
    public MessageChannel cookingInputChannel() {
        return new DirectChannel();
    }
    ...
    @ServiceActivator(inputChannel = "cookingInputChannel")
    public void messageReceiver1(String payload) {
        if (!Controller.publishedMessage.equals(payload)) {
            System.out.println("Message arrived! Cooking Payload: " + payload);
        }
    }
