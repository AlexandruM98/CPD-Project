...
public class Config {
    ...
    public PubSubInboundChannelAdapter sportChannelAdapter(
        ...
        PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "Sport-sub2");
    ...
    @Bean
    public PubSubInboundChannelAdapter newsChannelAdapter(
            @Qualifier("newsInputChannel") MessageChannel inputChannel,
            PubSubTemplate pubSubTemplate) {
        PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "News-sub2");
        adapter.setOutputChannel(inputChannel);
    ...
    @Bean
    public MessageChannel newsInputChannel() {
        return new DirectChannel();
    }
    ...
    @ServiceActivator(inputChannel = "newsInputChannel")
    public void messageReceiver1(String payload) {
        if (!Controller.publishedMessage.equals(payload)) {
            System.out.println("Message arrived! News Payload: " + payload);
        }
    }
